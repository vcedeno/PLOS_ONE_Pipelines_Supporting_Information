...
### -----------------------------
### Start.
def main(path,filename,foldermodel):
    ...
    rfilename=path+"/"+foldermodel+"/"+filename
    #print(rfilename)
    ...
    #interactionjson2=open(os.getcwd()+'/modeling-and-simulation-pipeline/json-schema/inputTransformation/datasets/static_markov/h1.json', 'w')
    ...
    interactionjsondata=[{"nd":foldermodel,"probabilities":[{"initial":str(row[0]),"values":[{"final":str(row2)}for row2 in row[1]]}]} for row in alljsondata]
    json.dump(interactionjsondata,interactionjson,indent=2)
    #json.dump(interactionjsondata,interactionjson2,indent=2)
    
    dynamicpath=os.getcwd()+'/property-inference-pipeline/test/results/h1/dynamic_markov/transition_probability_dynamic.csv'
    csvfile = open(dynamicpath, 'r')
    
    interactionjson=open(os.getcwd()+'/property-inference-pipeline/test/results/h1/dynamic_markov/h1.json', 'w')
    #interactionjson2=open(os.getcwd()+'/modeling-and-simulation-pipeline/json-schema/inputTransformation/datasets/dynamic_markov/h1.json', 'w')
    ...
    interactionjsondata=[{"nd":foldermodel,"probabilities":[{"vector":str(row[0]),"values":[{"v":str(row2)}for row2 in row[1]]}]} for row in alljsondata]
    json.dump(interactionjsondata,interactionjson,indent=2)
    #json.dump(interactionjsondata,interactionjson2,indent=2)
